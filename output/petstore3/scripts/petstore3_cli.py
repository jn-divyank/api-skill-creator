#!/usr/bin/env python3
"""
petstore3 CLI tool for Claude Code skill integration.
Provides access to the swagger-petstore-openapi-3-0 API.
"""
...
# --- Configuration ---
BASE_URL = _env.get("PETSTORE3_URL", "/api/v3")
AUTH_TOKEN = _env.get("PETSTORE3_TOKEN", "")

_ssl_ctx = ssl.create_default_context()
if _env.get("PETSTORE3_VERIFY_SSL", "true").lower() == "false":
...
def make_request(path: str, method: str = "GET", data: bytes = None, params: dict = None) -> tuple:
    """Make HTTP request to the petstore3 API."""
...
def cmd_check(args):
    """Verify connectivity to the petstore3 API."""
    if not BASE_URL:
        print(f"Error: PETSTORE3_URL not set", file=sys.stderr)
        sys.exit(1)
...
def cmd_pet_update_2(args):
    ...
    if hasattr(args, "data") and args.data:
        data = args.data.encode("utf-8")
    status, body = make_request(path, "POST", data=data, params=params)
    if status == 0 or status >= 400:
        print(f"Error (HTTP {status}): {body}", file=sys.stderr)
        sys.exit(1)
    try:
...
def main():
    parser = argparse.ArgumentParser(description="petstore3 CLI")
    ...
    p_pet_update_2.add_argument("--name", dest="name", default=None, help="Name of pet that needs to be updated")
    p_pet_update_2.add_argument("--status", dest="status", default=None, help="Status of pet that needs to be updated")
    ...
    p_pet_upload.add_argument("--additionalMetadata", dest="additionalMetadata", default=None, help="Additional Metadata")
    ...
    p_store_delete = sub_store.add_parser("delete", help="Delete purchase order by identifier.")
    ...
    p_user_create = sub_user.add_parser("create", help="Create user.")
    ...
    p_user_create_2.add_argument("--items", dest="items", default=None, help="items")
    ...
    p_user_update = sub_user.add_parser("update", help="Update user resource.")
    ...
    p_user_delete = sub_user.add_parser("delete", help="Delete user resource.")
