...
def generate_cli_script(spec, name, groups, multi_env=False, shell_source=False):
    ...
    # -- Shell environment loading (optional)
    if shell_source:
        lines.append('')
        lines.append('def _load_shell_environment() -> dict:')
        lines.append('    """Load environment variables from shell profile if not already set."""')
        lines.append('    env = os.environ.copy()')
        lines.append(f'    required = ["{prefix}_URL", "{prefix}_TOKEN"]')
        lines.append('    if all(k in env for k in required):')
        ...
        lines.append('')
        lines.append('')
        lines.append('_env = _load_shell_environment()')
    else:
        lines.append('')
        lines.append('_env = os.environ.copy()')
